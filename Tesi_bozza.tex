...
\begin{document}
	...
	\section{Source Contact Graph Routing}
		...
		% Immagine qui del pacchetto completo, forse è il caso di spiegare il pacchetto competo nel secondo capitolo?
		...
		Entrando nel dettaglio, l'algoritmo in questione può considerarsi un derivato della forma ricorsiva di una tipologia denominata {\bf DFS}, utilizzata per la ricerca nelle struttura ad albero o a grafo. {\bf DFS} è l'acronimo di {\it Depth-First Search}, cioè ricerca con priorità della profondità. 
		% nella  ricerca in grafi infatti..
		% questo tipo di rircarca si oppone alla ricerca in ampiezza blabla
		% il nostro obbiettivo si può ricondurre infatti ad una ricarca in un grafo con lo scopo di trovare un HS e segnarsi tutti i passi fatti per arrivarci
		%perchè la procedura funziona infatti partendo dal nodo destinazione
				...
				\Fn{\FRecurs{{\bf Indirizzo} destinazione, {\bf Percorso} finoAQui, {\bf TOV}}}{
					...
					\For{Per ogni contatto di questo nodo, da adesso fino al {\bf TOV}}
					{
						...
						\If{Se il contatto è compatibile con le specifiche}
						{
							...
							{
								{\bf TuttiPercorsi}.aggiungi(Nuovo{\bf Percorso});
							}
							\ElseIf{Se il nodo in contatto è un nanosatellite}
							{
								SCGR({\bf Indirizzo} di questo nodo, {\bf NuovoPercorso}, istante inizio questo contatto);
							}
						}
					}
				}
